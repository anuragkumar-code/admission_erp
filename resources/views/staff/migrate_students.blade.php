                            ...
                            <div class="bootstrap-data-table-panel">
                                <div class="table-responsive studenttabless">                                                                  
                                    <table class="table table-striped table-bordered" id="bootstrap-data-table-export">
                                        ...
                                        <tbody>
                                            <?php if($get_migrated_students ){ 
                                                foreach ($get_migrated_students as $key => $get_migrated_student) { ?>                                           
                                                <tr class="migration_row<?php echo $get_migrated_student->id; ?>">
                                                    <td>{{$key+1}}</td>
                                                    <td><?php if($get_migrated_student->priority == 1){ ?>
                                                        <span style="color: green"><strong>High</strong></span>
                                                        <?php }elseif($get_migrated_student->priority == 2){ ?>
                                                            <span style="color:orange"><strong>Medium</strong></span>
                                                        <?php }else{ ?>
                                                            <span style="color: red"><strong>Low</strong></span>
                                                        <?php } ?>
                                                    </td> 
                                                    <td><a style="color: blue" href="{{url('admin/student-details/'.base64_encode($get_migrated_student->id))}}">{{$get_migrated_student->first_name}} {{$get_migrated_student->middle_name}} {{$get_migrated_student->last_name}}
                                                    </a><i class='fa fa-times-circle' style='color:red'></i>
                                                       </td>

                                                    <td>{{$get_migrated_student->staff_name}} @if(Auth::user()->type==1 )  <?php if(isset($get_migrated_student->office_name)) {?>({{$get_migrated_student->office_name}}) <?php }?> @endif </td>

                                                    <td>{{$get_migrated_student->email}}</td>
                                                    <td>{{$get_migrated_student->phone}}</td>
                                                    <td>{{$get_migrated_student->country}}</td>       
                                                    <td>
                                                        <?php if($get_migrated_student->added_by == 1){?>
                                                            Self
                                                        <?php } else {?>
                                                            Admin
                                                        <?php } ?>
                                                    ...
                                                    <a href="javascript:void(0);" data-id="{{$get_migrated_student->id}}" class="migrate topicon01"><i style='font-size:22px !important;' class="fa fa-rocket" aria-hidden="true" title="Move to Prospects"></i></a>  


                                                        <a href="javascript:void(0);" data-id="{{$get_migrated_student->id}}" data-name="{{$get_migrated_student->first_name}}" data-phone="{{$get_migrated_student->phone}}"  data-dialcode="{{$get_migrated_student->phone_dialcode}}"  data-flag="{{$get_migrated_student->phone_flag}}"  data-toggle="modal" class="message topicon01 msgicons" data-target="#send_sms"><i style='font-size:22px !important' class="fa fa-comment-o"></i></a>    


                                                        <a href="javascript:void(0);" data-id="{{$get_migrated_student->id}}" data-name="{{$get_migrated_student->first_name}}" data-phone="{{$get_migrated_student->phone}}"  data-dialcode="{{$get_migrated_student->phone_dialcode}}"  data-flag="{{$get_migrated_student->phone_flag}}"  data-toggle="modal" class="message topicon01 msgicons" data-target="#send_whatsapp"><i style='font-size:22px !important'  class="fa fa-whatsapp" aria-hidden="true"></i></a> 


                                                        <a href="javascript:void(0);" data-id="{{$get_migrated_student->id}}" data-name="{{$get_migrated_student->first_name}}" data-phone="{{$get_migrated_student->phone}}" data-email="{{$get_migrated_student->email}}"  data-dialcode="{{$get_migrated_student->phone_dialcode}}"  data-flag="{{$get_migrated_student->phone_flag}}"  data-toggle="modal" class="message topicon01 msgicons" data-target="#send_mail"><i style='font-size:22px !important'  class="fa fa-envelope-o" aria-hidden="true"></i></a>    
                                                    ...
                                                    <div class="dropdown dropbtn">                                                       
                                                          ...
                                                          <div class="dropdown-menu dropmenu" aria-labelledby="dropdownMenuButton">
                                                            <a class="dropdown-item message" data-id="{{$get_migrated_student->id}}" href="javascript:void(0);" data-name="{{$get_migrated_student->first_name}}" data-toggle="modal" data-target="#myModal2">Client Notes</a>
                                                            <a href="" class="dropdown-item" data-toggle="modal" data-target="#myModal_{{$get_migrated_student->id}}">Checklist</a>
                                                            <a class="dropdown-item" href="{{url('/staff/migration_student_detail/'.base64_encode($get_migrated_student->id))}}">Migration details</a>
                                                            <a class="dropdown-item" href="{{url('admin/student-deleted/'.base64_encode($get_migrated_student->id))}}" onclick="return confirm('Are you sure you want to delete this student?');">Delete</a>
                                                          </div>
                                                    </div>
                                                    </td>
                                                </tr>
...
<div class="modal fade" id="myModal2" tabindex="-1" role="dialog" aria-labelledby="myModal2Label" aria-hidden="true">
  ...
  <?php if($get_migrated_students){ 
    ...
    <div class="modal fade" id="myModal_{{$get_migrated_student->id}}" role="dialog">
        <div class="modal-dialog">
          <div class="modal-content">   
            <div class="modal-header">
                <h5 class="modal-title" id="myModal2Label">Lead Status of {{$get_migrated_student->first_name}} {{$get_migrated_student->last_name}}</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true">&times;</span>
                </button>
              </div>    
            <div class="modal-body">
                <p>
                    <strong>
                        ...
                        <span class="float-right">
                            @if($get_migrated_student->email !='' && $get_migrated_student->is_verified==1 )
                ...
                <p>
                    <strong>
                        Passport
                        <span class="float-right">
                            @if($get_migrated_student->passport_number != '' )
                            <i class="fa fa-check" style="color: green" aria-hidden="true"></i>
                            @else<i class="fa fa-times" style='color: red' aria-hidden="true"></i>@endif
                        </span>
                    </strong>
                ...
                <p>
                    <strong>
                        Visa
                        <span class="float-right">
                            @if($get_migrated_student->visa_type != '' )
                            <i class="fa fa-check" style="color: green" aria-hidden="true"></i>
                            @else<i class="fa fa-times" style='color: red' aria-hidden="true"></i>@endif
                        </span>
                    </strong>
                ...
                <p>
                    <strong>
                        OSHC/OVHC
                        <span class="float-right">
                            @if($get_migrated_student->oshc_ovhc_file != '' )
                            <i class="fa fa-check" style="color: green" aria-hidden="true"></i>
                            @else<i class="fa fa-times" style='color: red' aria-hidden="true"></i>@endif
                        </span>
                    </strong>
                 ...
                 <p>
                    <strong>
                        IELTS / PTE Score
                        <span class="float-right">
                            @if($get_migrated_student->ielts_pte_score_file != '' )
                            <i class="fa fa-check" style="color: green" aria-hidden="true"></i>
                            @else<i class="fa fa-times" style='color: red' aria-hidden="true"></i>@endif
                        </span>
                    </strong>
...
$('.phone_number .thereismeta').addClass('<?php if(isset($get_migrated_student->phone_flag)){ echo $get_migrated_student->phone_flag; } ?>');
$('.phone_number .iti__selected-dial-code').html('<?php if(isset($get_migrated_student->phone_dialcode)){ echo $get_migrated_student->phone_dialcode; } ?>');
...
$('.whatsapp .thereismeta').addClass('<?php if(isset($get_migrated_student->whatsapp_flag)){ echo $get_migrated_student->whatsapp_flag; } ?>');
$('.whatsapp .iti__selected-dial-code').html('<?php if(isset($get_migrated_student->whatsapp_dialcode)){ echo $get_migrated_student->whatsapp_dialcode; }?>');
